...
// 导入主题工具函数
import { THEMES, getCurrentTheme, applyTheme, toggleTheme, isDarkTheme } from './util.theme.js';
...
const DEFAULT_SETTINGS = {
	notify: false,
	sound: false,
	theme: 'dark' // Material You dark theme as default
...
function setupSettingsPanel() {
		...
		<div class="settings-section">
			...
			<div class="settings-item">
				<div class="settings-item-label">
					...
					<div class="settings-item-description">${settings.theme === THEMES.DARK ? t('settings.theme_dark', 'Dark theme active') : t('settings.theme_light', 'Light theme active')}</div>
				</div>
				<label class="switch">
					<input type="checkbox" id="settings-theme-toggle" ${settings.theme === THEMES.DARK ? 'checked' : ''}>
					<span class="slider"></span>
				</label>
			</div>
		</div>
	...
	// 主题切换事件处理
	const themeToggle = $('#settings-theme-toggle', settingsContent);
	if (themeToggle) {
		on(themeToggle, 'change', e => {
			...
			const description = e.target.closest('.settings-item').querySelector('.settings-item-description');
			if (description) {
				description.textContent = newTheme === THEMES.DARK ? 
					t('settings.theme_dark', 'Dark theme active') : 
					t('settings.theme_light', 'Light theme active');
			}
		});
	}
}
